...
import { 
  ...
  Camera, Info, ShieldAlert, History, FileText, BookmarkCheck, Layout
} from 'lucide-react';
...
const Visa: React.FC<Props> = ({ language }) => {
  ...
  const [formData, setFormData] = useState<Partial<Application>>({
    ...
    email: '', phone: '',
    passport_file: '', photo_file: '', payment_receipt_file: ''
  });
  ...
  const handleApplyFinal = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const newApp: Application = {
        ...
        currency: 'VND',
        support_files: [formData.passport_file || ''].filter(f => f !== '')
      };
    ...
    <div className="max-w-screen-xl mx-auto px-6 py-16 space-y-24 animate-in fade-in duration-700">
      ...
      <section className="bg-white rounded-[3rem] border border-slate-100 shadow-xl overflow-hidden">
        <div className="bg-red-600 p-8 md:p-12 text-white text-center space-y-4">
          ...
          <p className="text-lg md:text-xl font-medium opacity-90 max-w-4xl mx-auto italic">
            {t.visaIntro[language]}
          </p>
        </div>
        
        <div className="p-8 md:p-16 grid md:grid-cols-2 lg:grid-cols-3 gap-10">
          <div className="space-y-4">
            <div className="flex items-center gap-3 text-red-600"><Layout className="w-6 h-6" /><h3 className="font-black uppercase tracking-widest text-sm">{t.visaTypes[language]}</h3></div>
            <p className="text-slate-600 text-sm leading-relaxed">{t.visaTypesText[language]}</p>
          </div>
          <div className="space-y-4">
            <div className="flex items-center gap-3 text-red-600"><ShieldCheck className="w-6 h-6" /><h3 className="font-black uppercase tracking-widest text-sm">{t.visaRules[language]}</h3></div>
            <p className="text-slate-600 text-sm leading-relaxed">{t.visaRulesText[language]}</p>
          </div>
          <div className="space-y-4">
            <div className="flex items-center gap-3 text-red-600"><History className="w-6 h-6" /><h3 className="font-black uppercase tracking-widest text-sm">{t.visaDuration[language]}</h3></div>
            <p className="text-slate-600 text-sm leading-relaxed">{t.visaDurationText[language]}</p>
          </div>
          <div className="space-y-4">
            <div className="flex items-center gap-3 text-red-600"><CreditCard className="w-6 h-6" /><h3 className="font-black uppercase tracking-widest text-sm">{t.visaFees[language]}</h3></div>
            <p className="text-slate-600 text-sm leading-relaxed">{t.visaFeesText[language]}</p>
          </div>
          <div className="space-y-4">
            <div className="flex items-center gap-3 text-red-600"><BookmarkCheck className="w-6 h-6" /><h3 className="font-black uppercase tracking-widest text-sm">{t.visaWhy[language]}</h3></div>
            <p className="text-slate-600 text-sm leading-relaxed">{t.visaWhyText[language]}</p>
          </div>
          <div className="space-y-4">
            <div className="flex items-center gap-3 text-red-600"><FileText className="w-6 h-6" /><h3 className="font-black uppercase tracking-widest text-sm">{t.visaLegal[language]}</h3></div>
            <p className="text-slate-600 text-sm leading-relaxed">{t.visaLegalText[language]}</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-slate-50 p-12 md:p-20 rounded-[4rem] border border-slate-200 shadow-inner relative overflow-hidden">
         <div className="text-center mb-16 space-y-4 relative z-10">
           <h2 className="text-4xl font-black text-slate-900 uppercase tracking-tight">{t.visaStatusCheck[language]}</h2>
           <p className="text-slate-500 font-medium italic">Authenticate entry documents or track pending application progress.</p>
         </div>
        ...
        {!showApply ? (
          <div className="text-center space-y-8">
            <div className="bg-white p-12 rounded-[3rem] border shadow-sm max-w-2xl mx-auto">
              <h3 className="text-2xl font-black text-slate-900 uppercase mb-4">{t.visaApplyNow[language]}</h3>
              <p className="text-slate-500 italic mb-8">Start your electronic visa application process. Ensure your passport is valid and ready for scan.</p>
              <button 
          ...
          <div className="bg-white p-12 md:p-20 w-full rounded-[4rem] shadow-2xl border border-slate-100 relative overflow-hidden text-slate-900">
              ...
              <div className="space-y-12 animate-in slide-in-from-right-8 duration-300">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-2">
                    ...
                    <input required value={formData.nationality} onChange={e => setFormData({...formData, nationality: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-100 p-5 rounded-3xl text-slate-900 font-bold outline-none focus:border-red-600 uppercase" placeholder="e.g. United Kingdom" />
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <input required value={formData.details.visaInfo} onChange={e => setFormData({...formData, details: {...formData.details, visaInfo: e.target.value}})} className="w-full bg-slate-50 border-2 border-slate-100 p-5 rounded-3xl text-slate-900 font-bold outline-none focus:border-red-600" placeholder="Purpose / Type" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase ml-2">{t.contactUs[language]}</label>
                    <input required value={formData.details.contactDetails} onChange={e => setFormData({...formData, details: {...formData.details, contactDetails: e.target.value}})} className="w-full bg-slate-50 border-2 border-slate-100 p-5 rounded-3xl text-slate-900 font-bold outline-none focus:border-red-600" placeholder="Phone or Social ID" />
                  </div>
                </div>
            ...
            {step === 2 && (
              <div className="space-y-12 animate-in slide-in-from-right-8 duration-300">
                <div className="text-center"><h3 className="text-3xl font-black text-slate-900 uppercase tracking-tight">Step 2: File Uploads</h3></div>
            ...
            {step === 4 && (
              <div className="text-center py-24 space-y-12 animate-in zoom-in duration-500">
                ...
                <h3 className="text-4xl font-black text-slate-900 uppercase tracking-tighter">Visa Docket Created</h3>
